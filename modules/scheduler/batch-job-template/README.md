...
| <a name="input_image"></a> [image](#input\_image) | Google Cloud Batch compute node image. Ignored if `instance_template` is provided. | <pre>object({<br>    family  = string<br>    project = string<br>  })</pre> | <pre>{<br>  "family": "fedora-37-base-image",<br>  "project": "cloud-hpc-image-public"<br>}</pre> | no |
